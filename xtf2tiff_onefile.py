...
def convert_xtf_tiff(file_path: str, output_folder_path: str, output_bitdepth: int, resize_half_width: bool, histogram_equalization: bool, column_threshold: int):
    ...
    if XTFHeaderType.sonar in p:
        ...
        average_along_columns = np.mean(image_array, axis=0)
        #empty_columns = np.where(average_along_columns < column_threshold)[0]

        # Delete 1793
        image_array = np.delete(image_array, np.s_[1:1794], axis=1)
        ...
        #np_chan = np.delete(np_chan, np.s_[1:1794], axis=1)
        ...
        plt.title('Value over ping datapoints')
        plt.xlabel('X')
        plt.ylabel('Value')
        plt.grid(True)
        plt.show()

        print("Columns after cleanup:", image_array.shape[1])
        ...
        print(f"Saving file {output_folder_path / output_filename}, width {im2.size[0]}, height {im2.size[1]}")
        im2.save(output_folder_path / output_filename)
